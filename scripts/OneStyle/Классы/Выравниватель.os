...
Процедура СвернутьПробелы( пСтрока, Знач пФрагмент, Знач пВыравнивание )
	...
	ИначеЕсли пВыравнивание = "Лево" Тогда
		...
		Если пФрагмент = "("
			И Не ДобавлятьПробелыВнутриСкобок() Тогда
...
Функция СформироватьОтступПеренесеннойСтроке( Знач пТекСтрока,
	...
	отступы       = СтрЗаменить( ПолучитьОтступы( пПредСтрока ) , "	", "    " );
	...
	Если КодЗаканчиваетсяНа( пПредСтрока, "," ) Тогда
		...
		ИначеЕсли пЭтоПерваяСтрокаВВыравнивании Тогда
			...
			количествоПробелов = длинаОтступов + 4;
			
		Иначе
			
			сдвигЗнач          = ПолучитьСдвигЗнач( пТекСтрока, пПредСтрока );
			количествоПробелов = длинаОтступов + сдвигЗнач;
		...
		строкаПробелов   = СтрокаПробелов( количествоПробелов );
		новыйОтступ      = ТабыВместоПробелов( строкаПробелов );
