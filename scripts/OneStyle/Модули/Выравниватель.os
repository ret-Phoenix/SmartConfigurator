...
// TODO: Учет перенесенных выражений при обрамлении пустыми строками
// TODO: выравнивание уже существующих перенесенных выражений
// TODO: Определение позиции выравнивания с учетом заданных значений по умолчанию
//Функция ТолькоЦифрыВСтроке( Знач Значение,
//                            Знач УчитыватьЛидирующиеНули = Истина,
...
Процедура РазбитьДлинныеВыражения( пТекСтрока, строкаИзменена, новыйКод, режим = 0 )
	...
	// Если строка начинается с разделителя, то пропускаем его
	строкаДляПоиска = Сред( пТекСтрока, длинаОтступов + 2 );
	...
	Если режим = 0
		...
		списокРазрывов.Добавить( ПозицияФрагмента( строкаДляПоиска, "," ), "," );
	...
	Если режим = 0
		...
		списокРазрывов.Добавить( ПозицияФрагмента( строкаДляПоиска, "И", Истина ), "И" );
		списокРазрывов.Добавить( ПозицияФрагмента( строкаДляПоиска, "ИЛИ", Истина ), "ИЛИ" );
	...
	Для каждого цРазрыв Из списокРазрывов Цикл
		...
		Если цРазрыв.Представление = "," Тогда
			...
			позицияРазрыва  = длинаОтступов + цРазрыв.Значение + 1 + СтрДлина( цРазрыв.Представление );
			...
			позицияПоследнегоСлова = ПозицияПоследнегоСлова( строкаДоРазрыва );
			
			Если позицияПоследнегоСлова > 0 Тогда
				...
				строкаПробелов = СтрокаПробелов( позицияПоследнегоСлова - длинаОтступов - сдвигЗнач );
			...
			пТекСтрока = отступы + строкаПробелов + строкаПослеРазрыва;
			новРежим   = 1;
...
Функция ПозицияПоследнегоСлова( Знач строкаДоРазрыва )
	
	длинаСтроки            = СтрДлина( строкаДоРазрыва );
	позицияПоследнегоСлова = 0;
	найденоСлово           = Ложь;
	
	Для ц = 1 По длинаСтроки Цикл
		
		текПозиция = длинаСтроки - ц;
		
		КодСимвола = КодСимвола( строкаДоРазрыва, текПозиция );
		
		Если ЭтоРазделительСлов( КодСимвола ) Тогда
			
			Если найденоСлово Тогда
				
				позицияПоследнегоСлова = текПозиция;
				Прервать;
	...
	Возврат позицияПоследнегоСлова;
