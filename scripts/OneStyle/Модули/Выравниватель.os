...
//TODO: Удаление лишних пробелов вокруг ключевых слов
// Если   Условие   Тогда
// п1 = 2   ;

//TODO: При добавлении пустых строк внутри блока неверно рассчитывается отступ
...
Функция ПозицияРазрыва( Знач пТекСтрока, Знач пФрагмент, Знач пИскатьОтдельноеСлово = Ложь )
	...
	Если СтрНачинаетсяС( СокрЛ( ТЕКСТРОКА ), ФРАГМЕНТ ) Тогда
		...
		позицияФрагмента = ПозицияФрагмента( Сред( ТЕКСТРОКА, первоеУпоминание + СтрДлина( ФРАГМЕНТ ) ), ФРАГМЕНТ, пИскатьОтдельноеСлово );
...
Функция КодЗаканчиваетсяНа( Знач пСтрокаКода, Знач пФрагмент, Знач ЭтоСлово = Ложь )
	...
	Возврат ПозицияФрагмента( КОД, ФРАГМЕНТ, ЭтоСлово ) = СтрДлина( КОД );
...
Процедура ФорматироватьПереносы( пИсходныйКод )
	...
	Для цНомерСтроки = 1 По пИсходныйКод.КоличествоСтрок() Цикл
		...
		строкаИзменена = УстановитьОтступПеренесеннымСтрокам(цТекСтрока, предСтрока, этоСерияПеренесенныхВыражений) ИЛИ строкаИзменена;
...
Функция УстановитьОтступПеренесеннымСтрокам( пТекСтрока, Знач предСтрока, этоСерияПеренесенныхВыражений)
	...
	Если СтрЗавершеннаяСтрока( предСтрока ) Тогда
...
Функция ДобавитьПустуяСтрокуПосле( Знач пТекСтрока, Знач пПредСтрока, пОтложеннаяУстановка )
	...
	Если добавитьПустуюСтроку Тогда
		...
		Если Не СтрЗавершеннаяСтрока( пПредСтрока ) Тогда
...
Функция СформироватьОтступПеренесеннойСтроке( Знач пТекСтрока, Знач пПредСтрока, Знач пЭтоПерваяСтрокаВВыравнивании = Ложь )
