...
Процедура ОбработатьМодуль(СтрокаРВ)
	
	РаботаСОкнами = Новый РаботаСОкнами();
	РаботаСОкнами.ЗапомнитьТекущееОкно();
	
	Данные = ПолучитьТекстИзОкна();
	СписокДляВыбора = ПолучитьСтрокиПоРегВыражению2(Данные, СтрокаРВ);
	...
	НомСтр = ВыборИзСписка.ВыбратьИзСписка(СписокДляВыбора, Истина);
...
Функция ИмяСекции(Данные) Экспорт
	...
	Для А = -КолСтрок по 0 Цикл
		...
		Если Совпадения.Количество() > 0 Тогда
			Для Каждого Сп Из Совпадения Цикл
				СпГр = Сп.Группы;
				// лог.ДОбавить("нашли имя секции::" + СпГр);
