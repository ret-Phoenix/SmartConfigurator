...
Процедура ПоискРегулярныеВыражения()
	Данные = ПолучитьТекстИзОкна();
	ДанныеВыбора = ""
...
Процедура ПерейтиВНачалоМетода()
...
Процедура ПоказатьИмяМетода()
	Данные = Обмен.ПолучитьТекстИзФайла("tmp\module.txt");
	ИмяМетода = ИмяМетода(Данные);
	Обмен.ВвестиЗначение(ИмяМетода);
КонецПроцедуры

Процедура ВыбратьРанееНабранноеСлово()
