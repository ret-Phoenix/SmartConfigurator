...
Функция ПолучитьНомерСтрокиНачалаМетода(Данные)
	...
	Для А = -КолСтрок По 0 Цикл
		...
		Если РегВыражение.Совпадает(Стр) Тогда
			Возврат (- А + 1);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПоказатьИмяМетода() Экспорт
	...
	Если ИмяМетода = Неопределено Тогда
		Обмен.Предупреждение("Имя метода не определено. Проверьте определения метода.");
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьИмяСекции() Экспорт
	...
	Если ИмяСекции = Неопределено Тогда
		Обмен.Предупреждение("Имя секции не определено. Проверьте наличие секции.");
		Возврат;
	КонецЕсли;

	Обмен.ВвестиЗначение(ИмяСекции);
...
Процедура ПриСозданииОбъекта()
	...
	Если Действие = "НачалоМетода" Тогда
		ПерейтиВНачалоМетода();
	ИначеЕсли Действие = "КонецМетода" Тогда
		ПерейтиВКонецМетода();
	ИначеЕсли Действие = "СписокМетодов" Тогда
		СписокМетодов();
	ИначеЕсли Действие = "СписокОбластей" Тогда
		СписокОбластей();
	ИначеЕсли Действие = "RegExSearch" Тогда
		ПоискРегулярныеВыражения();
	ИначеЕсли Действие = "РезультатПоследнегоПоиска" Тогда
		РезультатПоследнегоПоиска();
	ИначеЕсли Действие = "ПоказатьПоследнийСписокВыбора" Тогда
		ПоказатьПоследнийСписокВыбора();
	ИначеЕсли Действие = "ИмяМетода" Тогда
		ПоказатьИмяМетода();
	ИначеЕсли Действие = "ИмяСекции" Тогда
		ПоказатьИмяСекции();
	ИначеЕсли Действие = "ВыборИмениМетода" Тогда
		ВыбратьИмяМетода();
	ИначеЕсли НРег(Действие) = "allwords" Тогда
		ВыбратьРанееНабранноеСлово();
	Иначе
		Обмен.Предупреждение("Работа скрипта прекращена. Не определено действие: " + Действие);
	КонецЕсли;
