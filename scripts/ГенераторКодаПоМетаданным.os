...
Процедура ГенерироватьКодСоздания(ВариантГенерации)
	...
	Для Каждого Стр Из ОбъектыМетаданных Цикл
		А = А + 1;
		ДобавитьДляВыбора(СписокВыбора, Стр, А);
	КонецЦикла;
	...
	Если ДанныеМД = "" Тогда
		_Лог.Добавить("Нет информации по метаданным");
		Возврат;
	КонецЕсли;
	...
	Если (ТипМД = "Документ") или (ТипМД = "Справочник") Тогда
		...
		ИначеЕсли (ТипМД = "Справочник") Тогда
	...
	Иначе
		_Лог.Добавить("Обработаем регистры");
		...
		_Лог.Добавить("Результат::: " + Результат);
...
Процедура ГенерацияКодаПоМетаданным()
	...
	_Лог.Добавить("ГенерацияКодаПоМетаданным()");
