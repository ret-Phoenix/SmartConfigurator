...
- Добавление ссылки на основной реквизит в модуле (преобразование модуля формы из обычных форм в управляемые)
- Генератор кода для объектов метаданных (требуется предварительная генерация файла структуры обработкой v8CfgAddsAhk\ext\ПолучениеСтруктуры.epf)
- Менеджер окон: {Win+W}
...
Запуск
---
v8CfgAddsAhk\v8CfgAdds.ahk (Основной набор скриптов, раскладка при запуске значения не имеет)
